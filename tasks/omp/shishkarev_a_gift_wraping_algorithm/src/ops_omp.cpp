...
bool shishkarev_a_gift_wraping_algorithm_omp::TestTaskOpenMP::RunImpl() {
  ...
  #pragma omp parallel
  {
    ...
    #pragma omp critical
    {
      if ((input_[local_start].y < input_[start_point].y) ||
          ((input_[local_start].y == input_[start_point].y) && 
           (input_[local_start].x > input_[start_point].x))) {
  ...
  do {
    ...
    if (input_.size() <= 1) initial_candidate = p;
    else if (initial_candidate == p) initial_candidate = (p > 0) ? 0 : 1;
